...
export function WatchlistSettingsPopover() {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  const panelId = 'watchlist';
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        title="Watchlist notification settings"
      >
        <Settings size={12} />
      </button>
      ...
      {open && (
        ...
        >
          <h3 className="text-sky-400 font-bold tracking-widest mb-1 text-[10px]">
            DEFAULT SOUND
          </h3>
          <p className="text-[9px] text-radar-muted/70 mb-3">
            Used unless overridden per launch
          </p>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className={`w-9 h-5 rounded-full transition-colors relative flex-shrink-0 ${
                settings.enabled ? 'bg-sky-400' : 'bg-radar-border'
              }`}
            >
              <span
