...
export async function isLaunchAnnouncement(
  ...
  const userContent = [
    ...
    ``,
    `Say YES if the tweet explicitly states a project/token IS LAUNCHING, WILL LAUNCH, or HAS JUST GONE LIVE, even if it also contains airdrop bait, engagement farming ("RT + Like"), or hype language. The key signal is "launching on [platform/chain]", "launching [date]", "airdrop live", "mint is live", or "[ticker] LIVE" paired with a project/token name or ticker.`,
    ``,
    `Say NO for:`,
...
export async function isShillTweet(
  ...
  const userContent = [
    ...
    ``,
    `Say NO if the tweet:`,
    `- The tweet author's handle matches or closely resembles a $ticker or #hashtag mentioned in the tweet — this strongly indicates the project's OWN account, not a shill. For example, @fugabe tweeting about $FUGABE or #FUGABE is the project itself`,
